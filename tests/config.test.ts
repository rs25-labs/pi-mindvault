...
async function withTempConfig(fn: () => Promise<void>): Promise<void> {
  const prevPath = process.env.MINDVAULT_CONFIG_PATH;
  const prevProvider = process.env.MINDVAULT_EMBEDDINGS_PROVIDER;
  process.env.MINDVAULT_CONFIG_PATH = join(mkdtempSync(join(tmpdir(), "mvcfg-")), "config.json");
  delete process.env.MINDVAULT_EMBEDDINGS_PROVIDER; // exercise the real file/default, not the test-suite override
  try {
    await fn();
  } finally {
    if (prevPath === undefined) delete process.env.MINDVAULT_CONFIG_PATH; else process.env.MINDVAULT_CONFIG_PATH = prevPath;
    if (prevProvider === undefined) delete process.env.MINDVAULT_EMBEDDINGS_PROVIDER; else process.env.MINDVAULT_EMBEDDINGS_PROVIDER = prevProvider;
  }
}

test("loadConfig defaults to the local provider when no file exists", async () => {
...
test("MINDVAULT_EMBEDDINGS_PROVIDER env overrides the config file", async () => {
  await withTempConfig(async () => {
    const { loadConfig, writeConfig } = await import("../extensions/lib/config.ts");
    writeConfig({ embeddings: { provider: "local" } });
    process.env.MINDVAULT_EMBEDDINGS_PROVIDER = "hash";
